...
// docs/PLAN.md §A6 fixes the MCP surface at ten tools. Nothing else asserted the surface itself, so
// a tool silently renamed, dropped, or added (the generated SKILL.md names three of them by hand —
// see GeneratedSkillMatchesCliTests) was invisible to the suite.
public sealed class McpToolSurfaceTests
{
    private static readonly string[] expectedTools =
    [
        "cast_create", "cast_list", "cast_questions", "delegate", "delegate_async",
        "doctor", "job_result", "job_status", "list_backends", "list_roles",
    ];

    [Fact]
    public void ClaustrumToolsExposesExactlyTheTenPlannedTools()
    {
