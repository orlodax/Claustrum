...
claustrum jobs list [--last N]|show <id>|logs <id> [--stderr]|clean [--cwd <dir>]|budget <tree> [--reset]
...
- `claustrum doctor [--probe]`: per backend `binary` (path/version) · `auth` (env key or login file present — value never printed) · `probe` (1-token "reply OK" call, cost shown) · `mcp` (which config files register claustrum) · `os` (warns when binary path and cwd mix `C:\` and `/mnt/c`). The `probe` round trip (2026-09-21, issue #12) is a real `doctor-probe` role run through `Runner` on the cheapest configured alias that resolves to the backend, capped at $0.05; `CLAUSTRUM_SKIP_PROBE=1` turns every probe into a named skip — the test suite and CI set it so `dotnet test` never spends money.
...
- `max_parallel` is enforced by a per-cast semaphore in `JobManager` (both CLI and MCP paths go through it); a spawned architect that over-fans simply waits. `budget_usd` is enforced across the job tree: a child that would exceed the remaining budget is refused with status `BudgetExceeded`. (Shipped 2026-09-21, issue #9: a tree is named by `CLAUSTRUM_PARENT_JOB` — no variable, no tree; the ledger is `<claustrum home>/budget/<tree>/`, one entry per job, in-flight caps reserved through a held `.live` handle so a dead child never freezes the tree; a fan-out role's child is capped at `remaining / max_parallel`; a backend that reports no cost is charged its cap; `claustrum jobs budget <tree> [--reset]` inspects or clears it. NOTES.md "Tree budget accounting is a file ledger".)
...
3. **Smoke** (`scripts/smoke.ps1` / `smoke.sh`, extended 2026-09-21, issue #10): temp `git init` → `backends doctor` for every registered backend (absent ones SKIP, never FAIL) → `claustrum init` plus an idempotent rerun → for each installed builder-harness backend a paid `claustrum run builder --brief "create hello.txt containing hi" --json --cwd <tmp>` with the model from `CLAUSTRUM_SMOKE_MODEL_<NAME>` (claude defaults to `sonnet`; unset ⇒ SKIP) → assert `status=="success"` and `hello.txt ∈ changed_files` → a `max_parallel: 2` cast, two concurrent builders on distinct worktrees/branches, then `jobs clean`; table output; nonzero exit on any FAIL.
...
- M3: opencode `run --format json` event names; whether `openrouter/deepseek/<id>` is the exact three-segment form; Cursor `agent --output-format json` fields (measured 2026-09-21 — see §A3 row); Copilot `COPILOT_HOME` relocating `agents/`, non-interactive `--mode plan`, tool names for `--allow-tool`; Copilot's `.mcp.json` key shape.
- Cursor CLI cannot be smoke-tested on this machine (not installed, subscription unknown) — its backend ships behind fixtures + a teammate's validation. *(Resolved 2026-09-21: the owner installed `cursor-agent` on the Linux box; the backend is validated with real captures, Free plan ⇒ `--model auto` only.)*
